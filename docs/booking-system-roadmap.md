...
> **【実装済み】** このロードマップに沿った実装が `web/` にあります。
> Phase 0〜5 のうち、HP・お客様側の予約フロー・管理画面・メール通知まで実装が完了しています。
> セットアップ手順は [`web/README.md`](../web/README.md) を参照してください。
> 以下は、その設計判断の記録として残しています。
...
実装は `web/` に完了しているため、残りは「中身を自分のものに置き換えて公開する」作業です。

1. 上記「3-3. 事前に決めておくべきこと」を埋める
2. アカウント（Supabase / Cloudflare / Resend / ドメイン）を作成する
3. `web/src/config/site.ts` の `=== TODO` を実際の情報に書き換える（屋号・プロフィール・メニュー・営業日）
4. Supabase の SQL Editor で `web/supabase/migrations/0001_init.sql` を実行する
5. 写真を撮って `web/public/` に置く
6. デプロイする（手順は `web/README.md`）
